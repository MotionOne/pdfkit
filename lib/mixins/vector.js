...
export default {
  ...
  dash(length, options = {}) {
    ...
    // FIXED BY AMONSAGA 2024-06-05 { x > 0 -> x >= 0 }
  ...
  cornerRect(x, y, w, h, lt, rt, rb, lb) {

    lt = Math.min(lt, 0.5 * w, 0.5 * h);
    rt = Math.min(rt, 0.5 * w, 0.5 * h);
    rb = Math.min(rb, 0.5 * w, 0.5 * h);
    lb = Math.min(lb, 0.5 * w, 0.5 * h);

    // amount to inset control points from corners (see `ellipse`)
    const c_lt = lt * (1.0 - KAPPA);
    const c_rt = rt * (1.0 - KAPPA);
    const c_rb = rb * (1.0 - KAPPA);
    const c_lb = lb * (1.0 - KAPPA);

    this.moveTo(x + lt, y);
    this.lineTo(x + w - rt, y);
    this.bezierCurveTo(x + w - c_rt, y, x + w, y + c_rt, x + w, y + rt);
    this.lineTo(x + w, y + h - rb);
    this.bezierCurveTo(x + w, y + h - c_rb, x + w - c_rb, y + h, x + w - rb, y + h);
    this.lineTo(x + lb, y + h);
    this.bezierCurveTo(x + c_lb, y + h, x, y + h - c_lb, x, y + h - lb);
    this.lineTo(x, y + lt);
    this.bezierCurveTo(x, y + c_lt, x + c_lt, y, x + lt, y);
    return this.closePath();
  },
